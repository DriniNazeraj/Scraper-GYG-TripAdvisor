...
class Tripadvisor:
    ...
    def get_place(self, url):
        ...
        id_location = int(re.search('-d(\d+)-', url).group(1))
        geo_id = int(re.search('-g(\d+)-', url).group(1))
    ...
    def set_language(self, url, lang='ALL'):
        self.driver.get(url)
        self.driver.find_element(By.XPATH, '//label[@for=\'LanguageFilter_0\']').click()
        time.sleep(5)
    ...
    def get_reviews(self, page):
        ...
        if page > 1:
            self.driver.find_element(By.XPATH, '//a[@class=\'pageNum cx_brand_refresh_phase2 \' and contains(text(), {})]'.format(page)).click()
            time.sleep(2)
    ...
    def __expand_reviews(self):
        ...
        try:
            self.driver.find_element(By.XPATH, '//span[@class=\'_3maEfNCR\']').click()
